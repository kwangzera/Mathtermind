...
class DetectiveSolver(ClassicSolver):
    def gen_embed(self):
        if len(self.rounds) <= 4 and True not in self.verified:
            ...
            self.sol_panel.description = f"No verified guesses exist for solutions to be determined. Gueses 5 to 8 " \
                                         f"are guaranteed to be verified and guesses 1 to 4 can be verified by using " \
                                         f"the `;id` command. "
            return
